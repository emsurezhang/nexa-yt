---
name: nexa-yt
description: YouTube 内容采集工具集（关键词搜索 / 频道增量扫描 / 单视频详情与字幕抓取 / 订阅频道管理），以 MCP 风格 CLI 接口对外提供 JSON 输出。当任务需要从 YouTube 获取视频元数据、字幕文本、按关键词搜集视频、追踪频道最新视频、或管理订阅频道（增删改查/启停）时使用本技能；不适用视频下载、转码、上传等场景。
---
...
- 管理订阅频道：新增/删除/修改/启用/停用（读写 `config/channels.yaml`），按 alias 获取订阅频道视频列表
...
### 4. 订阅频道管理 — `src.yt_channel_manager`

订阅源即 `config/channels.yaml`（`url / alias / max_results / enabled`），管理操作直接读写该文件：

```bash
# 列出全部订阅频道
$PY -m src.yt_channel_manager list --output-stdout

# 新增频道（默认联网校验频道是否存在并自动补全 alias；--no-verify 跳过校验）
$PY -m src.yt_channel_manager add --url "https://www.youtube.com/@veritasium" \
  --alias "veri" --max-results 20 --output-stdout

# 更新 / 启停 / 删除（--ref 支持 alias 或 URL）
$PY -m src.yt_channel_manager update --ref veri --alias-new veritasium --max-results 30 --output-stdout
$PY -m src.yt_channel_manager disable --ref veri --output-stdout
$PY -m src.yt_channel_manager remove --ref veri --output-stdout

# 按 alias 获取订阅频道视频列表（委托频道扫描器，不更新增量游标）
$PY -m src.yt_channel_manager videos --ref veri --max-results 10 --since 2026-09-01 --output-stdout

# Agent 接口（--action list|add|remove|update|enable|disable|videos）
$PY -m src.yt_channel_manager agent --action add \
  --params-json '{"url": "https://www.youtube.com/@veritasium", "alias": "veri"}' --output-stdout
```

要点：
- 判重按 handle 归一化（`@Veritasium/videos` ≡ `@veritasium`）；alias 非空时全局唯一（大小写不敏感）。
- `remove` 会同步清理 `data/channel_state.json` 中该频道的增量游标。
- 错误（均退出码 2）：重复 / alias 冲突 → `ChannelAlreadyExistsError`；ref 定位失败 → `ChannelNotFoundError`。
- 非 stdout 模式结果写 `data/yt_channel_manage_<action>_<时间戳>.json`。
- 注意：写回 `channels.yaml` 后原有注释不保留（PyYAML 限制）。
...
### ChannelManagerOutput（频道管理）
```jsonc
{
  "generated_at": "ISO8601",
  "action": "list|add|remove|update|enable|disable",
  "channel": { "url", "alias", "max_results", "enabled" },  // 受影响的频道，list 时为 null
  "channels": [ /* 操作后的全量订阅列表，元素同 channel */ ]
}
```

### ChannelVideosOutput（订阅频道视频列表）
```jsonc
{
  "generated_at": "ISO8601",
  "channel": { "url", "alias", "max_results", "enabled" },
  "items": [ /* 同 YouTubeItem（extract_flat 平扫粒度） */ ],
  "failed_urls": []
}
```
...
- 全局限流（防 429）：`rate_limit.min_request_interval`（默认 15s）对**所有出站请求强制最小间隔，且跨 CLI 进程生效**（状态文件 `data/.rate_limit_state.json`）；字幕下载额外有 429 退避重试（`subtitle_max_retry` / `subtitle_retry_backoff`）。这些参数是防限流关键配置，不要随意调小。
- **批量抓取单视频详情时，优先用「程序化调用」**（同进程复用 fetcher 实例，见下文），循环间隔交给全局限流器，不要再用 shell 循环快速连续拉起多个 CLI 进程。
